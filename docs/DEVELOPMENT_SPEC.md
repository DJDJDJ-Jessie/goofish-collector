...
enabled + daily alarm
        │
        ├─ create monitor run ─► waiting
        │                          │
        │                          └─ open one link ► page-stable ► collect
        │                                             ├─ product detail + seller supplement
        │                                             └─ store profile + reviews + review images
        │
        ├─ write index/result/failure ─► next short alarm
        └─ last link ─► export product/store workbook ─► close tab ► next daily alarm

pause/delete ─► clear alarm + close monitor tab
```

监控配置对象采用以下最小结构；`lastRun*` 和 `nextRunAt` 只用于面板展示与恢复计划，不作为商品或店铺数据表字段：

```js
{
  id, kind: 'product' | 'store', name, links,
  hour, minute, downloadFolder, mode: 'rpa' | 'api', enabled,
  nextRunAt, lastRunAt, lastRunStatus, lastRunMessage,
  lastRunFile, lastRunCount, lastRunFailureCount
}
```
...
| `GET_MONITORS` | 读取商品/店铺监控配置、每日时间、运行进度和上次结果 |
| `SAVE_MONITOR` | 新建或更新一条监控，校验链接类型并创建每日闹钟 |
| `SET_MONITOR_ENABLED` | 启用或暂停监控，同步创建/清理每日闹钟；暂停时停止当前监控运行 |
| `DELETE_MONITOR` | 删除监控配置、清理闹钟并关闭当前监控专用标签页 |
| `RUN_MONITOR_NOW` | 不改变每日计划，立即启动一轮商品或店铺监控 |
